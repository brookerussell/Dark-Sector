...
all:
	@for i in $(SUBDIRS); do ( echo "" && echo "Compiling $$i..." && cd $(LARLITE_USERDEVDIR)/DarkSector/$$i && $(MAKE) ) || exit $$?; done
#####################################################################################
#
# CLEANs...
#
clean:
	@for i in $(SUBDIRS); do ( echo "" && echo "Cleaning $$i..." && cd $(LARLITE_USERDEVDIR)/DarkSector/$$i && $(MAKE) clean && rm -f $(LARLITE_LIBDIR)/$$i.* ) || exit $$?; done
...
doxygen:
	...
	@mkdir -p $(LARLITE_USERDEVDIR)/DarkSector/doc/dOxygenMyProject
	@doxygen $(LARLITE_USERDEVDIR)/DarkSector/doc/doxygenMyProject.script
